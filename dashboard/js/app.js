...
categoryForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // ============================
    if (file) {
      const cloudName = "dabky3wky";      // Your Cloudinary cloud_name
      const uploadPreset = "gimpex";      // Your unsigned preset
...
storeForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // ============================
    if (file) {
      const cloudName = "dabky3wky";
      const uploadPreset = "gimpex";
  ...
  const slug = document.getElementById("slug").value.trim();
    ...
    try {
      ...
      if (file) {
        const formData = new FormData();
        formData.append("file", file);
        formData.append("upload_preset", "gimpex");

        const res = await fetch(
          "https://api.cloudinary.com/v1_1/dabky3wky/image/upload",
